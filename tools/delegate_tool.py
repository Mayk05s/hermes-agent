...
def _normalize_specialist_id(value: Any) -> Optional[str]:
    ...
    if not text:
        return None
    if text in _HEALTH_SPECIALIST_CONTRACTS:
        return text
...
def _infer_specialist_from_goal(goal: Any) -> Optional[str]:
    ...
    for specialist in _HEALTH_SPECIALIST_CONTRACTS:
...
def _apply_specialist_contract(
    ...
    contract = _HEALTH_SPECIALIST_CONTRACTS.get(specialist)
    if not contract:
        return task
...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "- For health specialists, set specialist='fitness', 'nutrition', or "
        "'medical'. Hermes will enforce an isolated specialist contract, add "
        "the needed skill/file/health-db toolsets, and return granted/dropped "
        "toolsets in the result.\n"
...
DELEGATE_TASK_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "specialist": {
                "type": "string",
                "enum": ["fitness", "nutrition", "medical", "miniapp"],
                "description": (
                    "Optional first-class health specialist label. Use this "
                    "instead of relying only on prompt text when delegating to "
                    "telegram_health specialists. Hermes adds the specialist "
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "specialist": {
                            ...
                            "enum": ["fitness", "nutrition", "medical", "miniapp"],
                            "description": (
                                "Optional first-class health specialist label "
