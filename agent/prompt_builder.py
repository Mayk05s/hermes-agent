...
GOOGLE_CALENDAR_TOOL_GUIDANCE = (
    ...
    "availability, reminders attached to Calendar events, or a Calendar change), "
    "call `google_calendar` before answering. Resolve short follow-ups such as "
    "'again', 'nearby', or 'move it' from the conversation history and pass a "
    "complete, bounded task to the tool.\n"
    "A standalone request such as 'remind me at 22:00' is not a Calendar request "
    "unless the user explicitly asks to add it to Calendar. Use the available "
    "one-shot scheduler for that notification.\n"
